...
const SelectBillIds = (props) => {
    ...
    const handleKeyDown = (e) => {
        ...
        if (key === 38) {
            ...
            const selectedBillId =someIds[index]?someIds[index]?.caBSeq:listOfIds[index]?.caBSeq;
            ...
            if (dummyIds.includes(selectedBillId)) {
                ...
                setBillIdsObject(billObject.filter(id => id.caBSeq !== selectedBillId));
            } else {
    ...
    return (
        ...
        >
            <div className="modal-header d-block date_modal_header  pb-0" >
                ...
                <div className="row">
                    ...
                    <div className="col-lg-6 mt-2">
                        <div className="d-flex select-bill-box">
                            <div className="date-icons"><img src={cust_date} className="custom-date-icon" /></div>
                            <p className="date-range" onClick={openCustomDatePicker}>
                                {fromCustomDate ?
                                    moment(frDate).format("DD-MMM-YY") + " to " + moment(toDates).format("DD-MMM-YYYY") :
                                    dateValue ? dateValue :
                                        'Slect Date range'}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        {getBillids.map((item, index) => {
                            ...
                            return (
                                <div
                                    key={item.caBSeq}
                                    className={ isSelected || selectedIndex == index? 'd-flex row each-billId selected'
                                        : 'd-flex row each-billId'}
                                    onClick={() => handleSelectBillIds(item.caBSeq, item, index)}
                                >
                                    <div className="col-lg-6">
                                        <p>{item.caBSeq ? item.caBSeq : ''}</p>
                                        <p>{item.billDate ? item.billDate : ''}</p>
                                    </div>
                                    <div className="d-flex col-lg-6">
                                        <p>{item.amount ? item.amount : ''}</p>
                                        <div className="selected-img" >
                                            {isSelected ?
                                                <img src={selected_icon} width={"20px"} height={"20px"} /> : ''}
                                        </div>
                                    </div>
                                </div>
                            )
            ...
            {showCustDate ?
                <CustomDateSelection
                    ...
                    closeCustomDatePopUp={() => { seShowCustDate(false) }}
